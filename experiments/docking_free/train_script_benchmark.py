...
def get_dataset(model_name, root, split_method, split, seed=None, data_df=None, mmseqs_seq_clus_df=None):
    '''
    model: Literal['MGraphDTA', 'DGraphDTA', 'GraphDTA', 'AttentionDTA', 'DeepDTA']
    '''
    if model_name == 'MGraphDTA':
        from MGraphDTA.preprocessing import GNNDataset
        dataset = GNNDataset(df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df, root=root, split_method=split_method, split=split, seed=seed)
    elif model_name == 'DGraphDTA':
        ...
        dataset = Dataset(df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df, split_method=split_method, split=split, seed=seed)
    elif model_name == 'DeepDTA':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_df', type=str, default=f'{DATA_DIR}/davis_complete.csv', help='data of protein and ligand')
    parser.add_argument('--mmseqs_seq_clus_df', type=str, default=f'{DATA_DIR}/davis_complete_id50_cluster.tsv', help='mmseqs sequence cluster file')
    ...
    root = os.path.join(model_name, 'data/davis_complete/')
    data_df = pd.read_csv(args.data_df)
    mmseqs_seq_clus_df = pd.read_table(args.mmseqs_seq_clus_df, names=['rep', 'seq'])
    ...
    for model_seed in model_seeds:
        ...
        train_set = get_dataset(model_name=model_name, root=root, split_method=split_method, split='train', seed=model_seed, data_df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df)
        valid_set = get_dataset(model_name=model_name, root=root, split_method=split_method, split='valid', seed=model_seed, data_df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df)
        test_set = get_dataset(model_name=model_name, root=root, split_method=split_method, split='test', seed=model_seed, data_df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df)
    ...
    for model_seed in model_seeds:
        ...
        if not split_method == 'wt_mutation':
            test_wt_set = get_dataset(model_name=model_name, root=root, split_method=split_method, split='test_wt', seed=model_seed, data_df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df)
        test_mutation_set = get_dataset(model_name=model_name, root=root, split_method=split_method, split='test_mutation', seed=model_seed, data_df=data_df, mmseqs_seq_clus_df=mmseqs_seq_clus_df)
