...
class GNNDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        return [f'processed_data_{self.split_method}_all_protein{self.protein}_drug{self.drug}_cobminationseed_{self.seed}.pt',\
                f'processed_data_{self.split_method}_train_protein{self.protein}_drug{self.drug}_cobminationseed_{self.seed}.pt',\
                f'processed_data_{self.split_method}_test_protein{self.protein}_drug{self.drug}_cobminationseed_{self.seed}.pt',\
                f'processed_data_{self.split_method}_wt_protein{self.protein}_drug{self.drug}_cobminationseed_{self.seed}.pt']
    ...
    def process(self):
        ...
        smile_graph_path = os.path.join(self.root, 'raw', 'smile_graph.pkl')
        
        if not os.path.exists(smile_graph_path):
            smile_graph = dict()
            for smile in smiles:
                g = smile_to_graph(smile)
                smile_graph[smile] = g
            with open(smile_graph_path, 'wb') as file:
                pickle.dump(smile_graph, file)
        else:
            with open(smile_graph_path, 'rb') as file:
        ...
        self.all_list_ligand, self.all_list_protein = self.process_data(split_df['all'], smile_graph, target_graph)
        ...
        self.wt_all_list_ligand, self.wt_all_list_protein = self.process_data(split_df['wt_all'], smile_graph, target_graph)
        self.wt_test_list_ligand, self.wt_test_list_protein = self.process_data(split_df['wt_test'], smile_graph, target_graph)
        

    def __len__(self):
        if self.split == 'all':
            return len(self.all_list_ligand)
        elif self.split == 'train':
            return len(self.train_list_ligand)
        elif self.split == 'test':
            return len(self.test_list_ligand)
        elif self.split == 'wt_all':
            return len(self.wt_all_list_ligand)
        elif self.split == 'wt_test':
            return len(self.wt_test_list_ligand)
        else:
            raise ValueError("Unknown split: {}".format(self.split))

    def __getitem__(self, idx):
        if self.split == 'all':
            return self.all_list_protein[idx], self.all_list_ligand[idx]
        elif self.split == 'train':
            return self.train_list_protein[idx], self.train_list_ligand[idx]
        elif self.split == 'test':
            return self.test_list_protein[idx], self.test_list_ligand[idx]
        elif self.split == 'wt_all':
            return self.wt_all_list_protein[idx], self.wt_all_list_ligand[idx]
        elif self.split == 'wt_test':
            return self.wt_test_list_protein[idx], self.wt_test_list_ligand[idx]
        else:
            raise ValueError("Unknown split: {}".format(self.split))
